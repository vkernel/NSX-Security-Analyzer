...
docker run --rm --network none --entrypoint python vkernel/nsx-security-analyzer:0.2.0 -c 'import secrets; print("DJANGO_SECRET_KEY="+secrets.token_hex(32)); print("POSTGRES_PASSWORD="+secrets.token_hex(32)); print("WEB_PORT=8000")' > .env
chmod 600 .env
docker compose pull
docker compose up -d
